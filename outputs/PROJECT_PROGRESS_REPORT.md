...
| May 6, 2026 | Improved the Balance Story language selector with a custom dropdown displaying flag images, decluttered the top header by removing the Binance 1m OHLC badge, made the Futures DeskMate brand text clickable to reset the workspace, and added keyword-based FAQ search so agents can quickly find official references without extra filtering tabs. |
