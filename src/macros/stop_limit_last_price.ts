...
This usually happens because the market moved past your Limit Price too quickly. Setting the Stop and Limit to the *same price* ($${inputs.trigger_price}) gives no buffer for execution in volatile markets.`;
    ...
    zh: {
      title: "Stop-Limit · 未成交(Stop/Limit 同价 - Last)",
      formConfig: [
...
${inputs.triggered_at_utc} UTC+0 = **Last Price** 触及了您的 Stop 价格,您的限价单已被**触发**并挂入订单簿。
...
**"同价"的风险:**
将 Stop 与 Limit 设为相同价格,意味着您没有任何成交**缓冲空间**。限价单只能以该价格或更优的价格成交。在波动较大的加密市场中,这非常冒险。即便 Last Price 短暂触及触发价并立刻向不利方向移动,您的限价单也会因找不到对手方而一直挂单。

该分钟的价格如下,可以看出价格的变化:

${priceBlock}

数据显示,Last Price 触发后迅速远离了您的 Limit 价位,以致没有可以匹配该价格(或更优)的对手方,您的订单仍处于挂单(未成交)状态。
...
➡️ 您的 Stop-Limit 订单已**被触发**(由 Last Price)但**未成交**。
触发后 Last Price 迅速远离了您的 Limit 价位,且 Stop 与 Limit 设为相同价格 ($${inputs.trigger_price}) 没有任何缓冲,导致没有对手方匹配。`;
        }
      }
    },
    tr: {
